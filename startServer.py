...
def start_server(port=5000):
    ...
    if not os.path.exists('nodes.txt'):
        ...
        # Adiciona o nó ao arquivo, mas sem registro entre nós.
...
def start_multiple_servers():
    """Inicia múltiplos servidores e registra os nós entre si após todos estarem prontos."""
    ...
    ports = set(map(int, registered_ports)).union(default_ports)
    ...
    for port in ports:
        ...
        process.start()
        sleep(1)  # Atrasar o início de cada processo para evitar colisão de portas.
